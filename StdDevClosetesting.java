...
public class StdDevClosetesting implements IStrategy, IFeedListener {
    ...
    public Instrument[] testArray = {Instrument.GBPUSD};   
    ...
    public Period higherPeriod = Period.DAILY;
    @Configurable(value="Average Candle Size Min")
    public int averageCandleSizeMin = 40; //Min Candle Size requirement
    ...
    public double sTDevAverageMin = 75; //candle size min calculated into the average
    ...
    public void onStart(IContext context) throws JFException {
        ...
        dfOrder.setRoundingMode(RoundingMode.UP);   
        dfData.setRoundingMode(RoundingMode.CEILING);   
    }
   ...
   public void Optimizer(Instrument instrument) throws JFException{
        if (instrument.equals(Instrument.GBPUSD) || instrument.equals(Instrument.GBPAUD) || instrument.equals(Instrument.GBPCAD) || instrument.equals(Instrument.GBPCHF)){                           
            averageCandleSizeMin = 50; //Min Candle Size requirement    
            tpCandleSizeDev = 1.5;    
            highLowCheckSize = 27;    
            sTDevAverageMin = 42; //candle size min calculated into the average       
            minSL = 74;
            //orderAmount = Double.valueOf(df.format(orderAmount / 2));
        }
        if (instrument.equals(Instrument.USDCHF)){                           
            averageCandleSizeMin = 69; //Min Candle Size requirement    
            tpCandleSizeDev = 2.5;    
            highLowCheckSize = 27;    
            sTDevAverageMin = 15; //candle size min calculated into the average       
            minSL = 15;
        }
        if (instrument.equals(Instrument.XAUUSD)){
            orderAmount = Double.valueOf(dfOrder.format(orderAmount*10));
        }
    }
   ...
   public void AverageCandleSTDev(double sTDev, double average, String alteredOD, Instrument instrument) throws JFException{
        ...
        IBar higherCandle = history.getBar(instrument, higherPeriod, OfferSide.BID, 1);
        ...
        double cEMADouble = Double.valueOf(dfData.format(cEMA[0]));
        double[] cHEMA = indicators.ema(instrument, higherPeriod, OfferSide.BID, IIndicators.AppliedPrice.CLOSE, 21, Filter.WEEKENDS, 1, recentCandle.getTime(),0);
        double cHEMADouble = Double.valueOf(dfData.format(cHEMA[0]));
        
        Optimizer(instrument);
        //console.getOut().println("candleSize: " + candleSize + " 1 pip value: " + 1*instrument.getPipValue() + " avg candle size min: " + averageCandleSizeMin );
        
        if (candleSize > 0 && candleSize > averageCandleSizeMin){
            //console.getOut().println("Passed 1");
            if (recentCandle.getClose() > cEMADouble){
                //console.getOut().println("Passed 2");
                if (recentCandle.getClose() > (sTDevAndAverage * instrument.getPipValue()) + previousCandle.getClose() && HighLowCheck(instrument, true, highLowCheckSize) == true){
                    //console.getOut().println("Passed 3");
                    orderNumber++;                    
                    shortSL = Double.valueOf(dfData.format((recentCandle.getHigh() - (minSL * instrument.getPipValue()))));
                    shortTP = Double.valueOf(dfData.format(((candleSize / tpCandleSizeDev) * instrument.getPipValue()) + recentCandle.getHigh()));
                    entryPrice = recentCandle.getHigh();
                    for (int i = 1; i<6; i++){
                        IBar checkBar = history.getBar(instrument, myPeriod, OfferSide.BID, i);
                        if (entryPrice < checkBar.getHigh()){
                            entryPrice = checkBar.getHigh();
                        }
                    }
                    //if (higherCandle.getClose() > cHEMADouble){
                        //console.getOut().println("Passed 4");
                        engine.submitOrder("OrderN" + orderNumber + "_" + alteredOD, instrument, OrderCommand.BUYSTOP, orderAmount, entryPrice, slippageForOrder, shortSL, shortTP);                 
                    //}
                }
           ...
           if (recentCandle.getClose() < cEMADouble){
                if (recentCandle.getClose() < previousCandle.getClose() - (sTDevAndAverage * instrument.getPipValue()) && HighLowCheck(instrument, false, highLowCheckSize) == true){
                    ...
                    shortSL = Double.valueOf(dfData.format((recentCandle.getLow() + (minSL * instrument.getPipValue()))));
                    shortTP = Double.valueOf(dfData.format((recentCandle.getLow() - (candleSize / tpCandleSizeDev) * instrument.getPipValue())));
                    entryPrice = recentCandle.getLow();
                    for (int i = 1; i<6; i++){
                        IBar checkBar = history.getBar(instrument, myPeriod, OfferSide.BID, i);
                        if (entryPrice > checkBar.getLow()){
                            entryPrice = checkBar.getLow();
                        }
                    }                    
                    //if (higherCandle.getClose() < cHEMADouble){
                        engine.submitOrder("OrderN" + orderNumber + "_" + alteredOD, instrument, OrderCommand.SELLSTOP, orderAmount, entryPrice, slippageForOrder, shortSL, shortTP);                                     
                    //}
                } 
            }
        }
    ...
    public void onBar(Instrument instrument, Period period, IBar askBar, IBar bidBar) throws JFException {
        ...
        double eMA200Double = Double.valueOf(dfData.format(ema200[0]));
        ...
        double hMA200Double = Double.valueOf(dfData.format(hma200[0]));               
        ...
        */
        ...
        if (accountEQ < 100000){
            ...
            orderAmount = Double.valueOf(dfOrder.format(tempOrderNumber));
        }
        if (accountEQ > 100000){
